 ...
 *
 */
package org.example.mina.tls.common;
...
//    	 特点     :  密钥库和私钥用不同的密码进行保护
        KeyStore ks = KeyStore.getInstance("JKS"); // 创建keystore
        InputStream in = null;
        try {
        	// 当前类路径下获取证书
        ...
        // 初始化使用我们keystore的key manager factory
        KeyManagerFactory kmf = KeyManagerFactory.getInstance(KEY_MANAGER_FACTORY_ALGORITHM);
        kmf.init(ks, BOGUS_PW);

        // 初始化SSLContex与我们的key managers绑定
